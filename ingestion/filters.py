...
class CoordinateFilter(BaseFilter):
    """Rejects coordinates that are exactly 0,0 or out of range."""
    def filter(self, imei, record, last_record=None) -> bool:
        lat = record.get('latitude', 0.0)
        lng = record.get('longitude', 0.0)
        if lat == 0.0 or lng == 0.0:
            logger.warning(f"FILTERED [Coordinate]: Invalid 0,0 location for {imei}")
            return False
        if not (-90.0 <= lat <= 90.0) or not (-180.0 <= lng <= 180.0):
            logger.warning(f"FILTERED [Coordinate]: Coordinates out of range ({lat}, {lng}) for {imei}")
            return False
        return True
...
class TimeJumpFilter(BaseFilter):
    ...
    def __init__(self, max_future_seconds=1800, max_past_days=None):
        ...
        self.max_past_days = max_past_days if max_past_days is not None else getattr(Config, 'MAX_PAST_DAYS', 365)
