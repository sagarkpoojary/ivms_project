...
def get_filtered_vehicles(include_all=False):
    # We include session keys in memoize by making them arguments or using a unique key
    # But memoize already uses arguments. Let's pass session identifiers.
    return _get_filtered_vehicles_cached(
        session.get('email'), 
        session.get('role'), 
...
def _get_filtered_vehicles_cached(email, role, parent_email, include_all):
